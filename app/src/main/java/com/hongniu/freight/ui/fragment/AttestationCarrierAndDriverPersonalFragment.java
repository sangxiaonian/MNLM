 ...
 * 个人承运人身份认证第二部
 */
@Route(path = ArouterParamApp.fragment_attestation_carrier_and_driver_personal)
public class AttestationCarrierAndDriverPersonalFragment extends AttestationBaseFragment implements ItemTextView.OnCenterChangeListener, View.OnClickListener, ImageInforView.UpLoadFinishListener {
    ...
    private ImageInforView img_work;//从业资格证选填
    private ImageInforView img_driver_license;//身份证主页面
    private ImageInforView img_driver_license_second;//身份证反面


    @Override
    protected View initView(LayoutInflater inflater) {
        View inflate = inflater.inflate(R.layout.fragment_attestation_carrier_and_driver_personal, null);
        root = inflate;
        ...
        img_work = inflate.findViewById(R.id.img_work);
        img_driver_license = inflate.findViewById(R.id.img_driver_license);
        img_driver_license_second = inflate.findViewById(R.id.img_driver_license_second);
        return inflate;
    }
    ...
    protected void initData() {
        ...
        img_driver_license.setType(12);
        img_driver_license_second.setType(12);
        img_work.setType(15);

        img_work.setAttached(this);
        img_driver_license.setAttached(this);
        img_driver_license_second.setAttached(this);
        ...
        img_work.setUpLoadFinishListener(this);
        img_driver_license.setUpLoadFinishListener(this);
        img_driver_license_second.setUpLoadFinishListener(this);
    ...
    protected void initInfo(VerifyInfoBean verifyInfoBean) {
        ...
        VerifyPersonParams driver = verifyInfoBean.getDriver();
        ...
        if (driver!=null){
            img_driver_license.setImageInfo(driver.getFaceDLImageUrl());
            img_driver_license_second.setImageInfo(driver.getBackDLImageUrl());
            img_driver_license_second.setImageInfo(driver.getBackDLImageUrl());
            img_work.setImageInfo(driver.getQualificationCertificateImageUrl());
            item_number.setTextCenter(driver.getInputQcNum());
    ...
    private boolean check(boolean showAlert) {
        ...
        if (img_driver_license.getState() != 3) {

            if (showAlert) {
                ToastUtils.getInstance().show(getAlert(img_driver_license.getState(), "驾驶证主页"));
            }
            return false;
        }if (img_driver_license_second.getState() != 3) {

            if (showAlert) {
                ToastUtils.getInstance().show(getAlert(img_driver_license_second.getState(), "驾驶证副页"));
            }
            return false;
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.bt_sum) {
            if (check(true)) {
                ...
                params.setFaceDLImageUrl(img_driver_license.getImgInfo().getPath());
                params.setBackDLImageUrl(img_driver_license_second.getImgInfo().getPath());
                ...
                HttpAppFactory.verifyCarrierAndDriverPerson(params)
